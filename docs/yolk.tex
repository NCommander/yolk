...
\date{Revised October 19th. 2012}
...
to the application. There's a fully commented \emph{config.ini.dist}
file available in the \emph{extras/} directory. The only way to avoid
having \emph{config.ini} in the \emph{configuration/} directory is
if you manually change the path in the source file before compiling
Yolk.
...
the \emph{extras/rc.yolk} script gives you a rather nice and simple
...
When you call the \emph{Wait} procedure, you basically hang here until
\begin{enumerate}
...
function accepts a set of parameters that should be fairly self-explanatory.
...
Worth noting is the \emph{Set\_Dispatchers} parameter. This parameter
is access to a procedure which must handle adding \emph{all} the URI
and WebSocket dispatchers the application needs. There's currently
only support for URI dispatchers, so if your application needs any
of the other AWS dispatchers (Method, Linker, Timer etc. etc.) then
you will have to manage creating and starting/stopping the AWS component
manually.

Another potential manual operation is setting the \emph{Cache-Control}
header. This can become necessary if you
\begin{enumerate}
...
This does as expected: Starts the server.
...
\emph{Yolk.Server.Start} call was made.
...
This should preferably be called after having called \emph{Yolk.Server.Create},
since the call to \emph{Static\_Content\_Cache\_Setup} in \emph{Create}
