...
\date{Revised November 26th. 2012}
...
Yolk \emph{Version} string and a \emph{Config\_File} function to get
the location of the configuration file. These are used in a few places,
for example in the \emph{directory.tmpl} template file (the version
string) and in the \emph{Yolk.Configuration} package (the \emph{Config\_File}
function).
...
application using the SIGINT, SIGPWR and SIGTERM signals. You can
also get a PID file placed next to your executable (see the \emph{PID\_File}
configuration parameter), which coupled with the \emph{extras/rc.yolk}
